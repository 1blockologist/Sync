...
const string strMessageMagic = "Sync Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    else if (pindexBest->nHeight <= 93)
    {
      nSubsidy = 0 * COIN;
      return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight > 7199)
    {
      nSubsidy = 0 * COIN;
      return nSubsidy + nFees;
    }
...
{
    return ComputeMaxBits(bnProofOfStakeLimitAfterFork, nBase, nTime);
}
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Sync", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "16/05/2014";
        CTransaction txNew;
        txNew.nTime = 1400263615;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1400263615;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1261301;
		if(fTestNet)
        {
            block.nNonce   = 1261301;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xa87d9a593e3e65e96f44be39270e0d0e604e26f1fd567d72306579f3f34c19dd"));
        block.print();
